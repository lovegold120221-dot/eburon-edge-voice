...
def _profile_to_response(
    ...
    preset_alias = normalize_stored_profile_alias(getattr(profile, "preset_engine", None))
    default_alias = normalize_stored_profile_alias(getattr(profile, "default_engine", None))
...
def _validate_profile_fields(
    *,
    ...
    preset_alias = get_alias_entry(preset_engine)
    default_alias = get_alias_entry(default_engine)
    ...
    if voice_type == "preset":
        ...
        if not preset_alias or not is_active_alias(preset_alias):
            return f"Preset model '{preset_engine}' is not allowlisted for Eburon Edge Voice"
        return None
    ...
    if default_engine:
        if not default_alias or not is_active_alias(default_alias):
            return f"Cloned profiles cannot use default model '{default_engine}'"
        if not default_alias.voice_cloning_supported:
            return f"Cloned profiles cannot use default model '{default_engine}'"
    return None
...
async def create_voice_prompt_for_profile(
    ...
    if voice_type == "preset":
        ...
        preset_engine = resolve_upstream_engine(profile.preset_engine)
        if not preset_engine:
            raise ValueError(f"Preset profile {profile_id} is missing an allowlisted preset model")
        if preset_engine != engine:
            raise ValueError(
                f"Preset profile {profile_id} only supports engine '{preset_engine}', not '{engine}'"
            )
        return {
            "voice_type": "preset",
            "preset_engine": preset_engine,
