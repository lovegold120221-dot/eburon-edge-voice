...
from .. import config
from ..eburon_aliases import MODEL_PROVIDER, get_alias_for_generation
...
async def create_generation(
    ...
    return _serialize_generation_response(db_generation)
...
async def update_generation_status(
    ...
    return _serialize_generation_response(generation)
...
async def list_generations(
    ...
    for generation, profile_name in results:
        ...
        items.append(_serialize_history_response(generation, profile_name, versions, active_version_id))
...
def _serialize_generation_response(generation: DBGeneration) -> GenerationResponse:
    alias = get_alias_for_generation(generation.engine, generation.model_size)
    return GenerationResponse(
        ...
        engine=alias.alias_id if alias else None,
        model_size=alias.model_size if alias else None,
        provider=MODEL_PROVIDER,
        capabilities=list(alias.capabilities) if alias else [],
