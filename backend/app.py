...
def _configure_cors(application: FastAPI) -> None:
    ...
    default_origins = [
        "http://localhost:5173",  # Eburon Voice dev server
        "http://127.0.0.1:5173",
        "http://localhost:17493",  # Eburon Voice backend
        "http://127.0.0.1:17493",
        "http://localhost:17493/docs",  # Eburon Voice API docs
...
def _register_lifecycle(application: FastAPI) -> None:
    ...
    async def startup_event():
        ...
        logger.info("Eburon v%s starting up", __version__)
        logger.info(
            "Python %s on %s %s (%s)",
            sys.version.split()[0],
            platform.system(),
    ...
    async def shutdown_event():
        logger.info("Eburon server shutting down...")
        try:
