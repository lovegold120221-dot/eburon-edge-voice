...
from ..eburon_aliases import resolve_public_alias_or_raise
...
async def list_preset_voices(engine: str):
    ...
    entry = resolve_public_alias_or_raise(engine, action="preset-voice access")
    if entry.upstream_engine == "kokoro":
        from ..backends.kokoro_backend import KOKORO_VOICES

        return {
            "engine": entry.alias_id,
            "voices": [
                {
                    "voice_id": vid,
                    "name": name,
    ...
    return {"engine": entry.alias_id, "voices": []}
...
async def seed_preset_profiles_route(
    ...
    """
    entry = resolve_public_alias_or_raise(engine, action="preset seed")
    if entry.upstream_engine != "kokoro":
    ...
    try:
        ...
        for voice_id, display_name, gender, lang in KOKORO_VOICES:
            ...
            existing = (
                ...
                .filter_by(preset_engine=entry.alias_id, preset_voice_id=voice_id)
                .first()
            )
            if existing:
                continue
        ...
        return {"engine": entry.alias_id, "created": created, "total_available": len(KOKORO_VOICES)}
    except Exception as e:
...
async def export_profile(
    ...
    try:
        ...
        if not safe_name:
            safe_name = "profile"
        filename = f"profile-{safe_name}.eburon-voice.zip"
