...
from .. import models
from ..eburon_aliases import get_alias_entry_by_upstream_model, is_visible_alias
...
async def get_active_tasks():
    ...
    for raw_model_name in all_model_names:
        alias = get_alias_entry_by_upstream_model(raw_model_name)
        if not alias or not is_visible_alias(alias):
            continue

        public_model_name = alias.alias_id
        task = download_map.get(raw_model_name)
        progress = progress_map.get(raw_model_name)

        if task:
            error = task.error
            if not error:
                with progress_manager._lock:
                    pm_data = progress_manager._progress.get(raw_model_name)
                    if pm_data:
