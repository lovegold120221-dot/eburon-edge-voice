...
from ..eburon_aliases import (
    get_default_alias_id,
    log_policy_decision,
    normalize_stored_profile_alias,
    resolve_requested_alias_or_raise,
)
from ..services.generation import run_generation
from ..services.history import _serialize_generation_response
...
async def generate_speech(
    ...
    profile_default_alias = normalize_stored_profile_alias(getattr(profile, "default_engine", None))
    profile_preset_alias = normalize_stored_profile_alias(getattr(profile, "preset_engine", None))
    requested_alias = data.engine or profile_default_alias or profile_preset_alias or get_default_alias_id()

    try:
        alias = resolve_requested_alias_or_raise(
            requested_alias=data.engine,
            legacy_engine=data.engine,
            legacy_model_size=data.model_size,
            fallback_alias=profile_default_alias or profile_preset_alias or get_default_alias_id(),
            action="generate",
        )
        log_policy_decision(requested_alias=requested_alias, entry=alias, decision="allowed", request=request)
    ...
    engine = alias.upstream_engine
    model_size = alias.model_size
    ...
    generation = await history.create_generation(
        ...
        seed=data.seed,
        db=db,
        instruct=data.instruct if alias.upstream_engine == "qwen" else None,
...
async def retry_generation(generation_id: str, db: Session = Depends(get_db)):
    ...
    return _serialize_generation_response(gen)
...
async def stream_speech(
    ...
    model_size = alias.model_size or "default"
