...
from .. import models
from ..eburon_aliases import (
    alias_public_payload,
    get_alias_entry,
    get_alias_entry_by_upstream_model,
    get_registry_entries,
    log_policy_decision,
    resolve_public_alias_or_raise,
    translate_active_download_keys,
)
...
def _get_alias_entry_or_400(model_name: str):
    entry = get_alias_entry(model_name)
    if not entry:
...
async def unload_model_by_name(model_name: str):
    ...
    entry = _get_alias_entry_or_400(model_name)
    config = get_model_config(entry.upstream_model_name)
    if not config:
        raise HTTPException(status_code=400, detail=f"Unknown model: {model_name}")
...
async def get_model_progress(model_name: str):
    ...
    async def event_generator():
        async for event in progress_manager.subscribe(entry.upstream_model_name):
...
@router.get("/models", response_model=models.ModelStatusListResponse)
...
async def get_model_status():
    ...
    translated_active_downloads = translate_active_download_keys(active_download_names)
    ...
    from ..backends import check_model_loaded, get_model_config

    alias_entries = get_registry_entries()
    model_configs = []
        ...
        model_configs.append(
            {
                ...
                "check_loaded": lambda c=config: check_model_loaded(c),
            }
        )

    model_to_repo = {cfg["model_name"]: cfg["hf_repo_id"] for cfg in model_configs}
    active_download_repos = {
        model_to_repo.get(raw_name)
        for raw_name in translated_active_downloads.keys()
        if raw_name in model_to_repo
    }
    ...
    for config in model_configs:
        try:
            ...
            statuses.append(
                models.ModelStatus(
                    **alias_public_payload(config["alias"]),
                    hf_repo_id=None,
                    downloaded=downloaded,
                    downloading=config["model_name"] in translated_active_downloads or is_downloading,
                    size_mb=size_mb,
                    loaded=loaded,
                )
            )
...
async def trigger_model_download(request: models.ModelDownloadRequest):
    ...
    entry = resolve_public_alias_or_raise(request.model_name, action="download")
    config = get_model_config(entry.upstream_model_name)
    if not config:
    ...
    async def download_in_background():
        try:
            ...
            if asyncio.iscoroutine(result):
                await result
            task_manager.complete_download(config.model_name)
        except Exception as e:
            task_manager.error_download(config.model_name, str(e))

    task_manager.start_download(config.model_name)
    ...
    log_policy_decision(requested_alias=request.model_name, entry=entry, decision="allowed")
...
async def cancel_model_download(request: models.ModelDownloadRequest):
    ...
    task_manager = get_task_manager()
    progress_manager = get_progress_manager()
    entry = _get_alias_entry_or_400(request.model_name)
    raw_model_name = entry.upstream_model_name

    removed = task_manager.cancel_download(raw_model_name)

    progress_removed = False
    with progress_manager._lock:
        if raw_model_name in progress_manager._progress:
            del progress_manager._progress[raw_model_name]
