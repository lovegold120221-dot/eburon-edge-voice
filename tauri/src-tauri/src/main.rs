...
/// Find a eburon-server process listening on a given port (Windows only).
///
/// Uses PowerShell `Get-NetTCPConnection` to look up the PID owning the port,
/// then verifies via `tasklist` that it's a eburon process. The caller is
...
#[cfg(windows)]
fn find_eburon_pid_on_port(port: u16) -> Option<u32> {
...
async fn start_server(
    ...
    // Check if a eburon server is already running on our port (from previous session with keep_running=true)
    #[cfg(unix)]
    {
        use std::process::Command;
            ...
            .output()
        {
            ...
            for line in output_str.lines().skip(1) {
                ...
                if parts.len() >= 2 {
                    let command = parts[0];
                    let pid_str = parts[1];
                    if command.contains("eburon") {
                        if let Ok(pid) = pid_str.parse::<u32>() {
                            println!("Found existing eburon-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            // Port is in use — check if it's a eburon process
            if let Some(pid) = find_eburon_pid_on_port(SERVER_PORT) {
                println!("Found existing eburon-server on port {} (PID: {}), reusing it", SERVER_PORT, pid);
                *state.server_pid.lock().unwrap() = Some(pid);
                return Ok(format!("http://127.0.0.1:{}", SERVER_PORT));
            } else {
                return Err(format!(
                    "Port {} is already in use by another application. \
                     Close the other application or change the Eburon Voice port.",
                    SERVER_PORT
                ));
            }
        }
    ...
    // Kill any orphaned eburon-server from previous session on legacy port 8000
                ...
                if parts.len() >= 2 {
                    ...
                    if command.contains("eburon") {
                        if let Ok(pid) = pid_str.parse::<i32>() {
                            println!("Found orphaned eburon-server on legacy port {} (PID: {}, CMD: {}), killing it...", LEGACY_PORT, pid, command);
                        ...
                        println!("Legacy port {} is in use by non-eburon process: {} (PID: {}), not killing", LEGACY_PORT, command, pid_str);
                    }
                }
            }
        }
    ...
    #[cfg(windows)]
    {
        ...
        if TcpStream::connect_timeout(
            ...
            if let Some(pid) = find_eburon_pid_on_port(LEGACY_PORT) {
                println!("Found orphaned eburon-server on legacy port {} (PID: {}), killing it...", LEGACY_PORT, pid);
    ...
    println!("=================================================================");
    println!("Starting eburon-server sidecar");
    ...
    let cuda_binary = {
        ...
        if exe_path.exists() {
            ...
            {
                Ok(output) => {
                    // Output format: "eburon-server X.Y.Z\n"
    ...
    let sidecar_result = app.shell().sidecar("eburon-server");
        ...
        match tokio::time::timeout(tokio::time::Duration::from_millis(100), rx.recv()).await {
            ...
            Ok(None) => {
                ...
                #[cfg(not(debug_assertions))]
                {
                    ...
                    eprintln!("Check Console.app logs for more details (search for 'eburon')");
